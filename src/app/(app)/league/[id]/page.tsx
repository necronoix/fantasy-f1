          ...
          <Card>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-4xl font-black text-f1-red">{(nextGp as Record<string,unknown>).round as number}</span>
                <div>
                  <p className="font-bold text-white">{String((nextGp as Record<string,unknown>).name)}</p>
                  <p className="text-f1-gray text-xs">{String((nextGp as Record<string,unknown>).circuit)}</p>
                </div>
              </div>
              <div className="flex items-center justify-between text-xs text-f1-gray mt-3 pt-3 border-t border-f1-gray-dark">
                <span>Gara: <span className="text-white">{new Date(String((nextGp as Record<string,unknown>).date)).toLocaleDateString('it-IT', { day: '2-digit', month: 'long' })}</span></span>
                {(nextGp as Record<string,unknown>).has_sprint && <Badge variant="yellow">Sprint ⚡</Badge>}
              </div>
            </div>
          </Card>
        )}
